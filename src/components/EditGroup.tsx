...
import { z } from 'zod';
import type { HorarioDia, HorarioFormData, SessionGroup } from '../types';
...
interface EditGroupProps {
  onEditGroup: (data: HorarioFormData, id_grupo: number) => Promise<{ success: boolean; message?: string }>;
...
const EditGroup: React.FC<EditGroupProps> = ({ onEditGroup, selectedGroup }) => {
  const [formData, setFormData] = useState({ nombre_grupo: '' });
  ...
  useEffect(() => {
    if (selectedGroup) {
      setFormData({ nombre_grupo: selectedGroup.nombre_grupo });
      const initialActiveDays = new Map<number, { inicio: string, fin: string }>();
      if (selectedGroup.horarios) {
        selectedGroup.horarios.forEach(h => {
          initialActiveDays.set(h.dia_semana, { inicio: h.hora_inicio, fin: h.hora_fin });
        });
      }
  ...
  const handleDayToggle = (diaId: number, isChecked: boolean) => {
    setActiveDays(prev => {
      ...
      if (isChecked) {
        newMap.set(diaId, { inicio: '09:00', fin: '17:00' });
      } else {
        newMap.delete(diaId);
      }
      return newMap;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const horariosParaEnviar: HorarioDia[] = Array.from(activeDays.entries()).map(([diaId, horas]) => ({
        dia_semana: diaId,
        hora_inicio:formatTimeWithoutSeconds(horas.inicio),
        hora_fin:formatTimeWithoutSeconds(horas.fin),
      }));
      ...
      console.log('horarioFormData edit',horarioFormData)
      const parsedHorarioFormData = horarioFormSchema.parse(horarioFormData);
      console.log('parsedHorarioFormData',parsedHorarioFormData)
      if (selectedGroup && selectedGroup.id_grupo) {
        await onEditGroup(parsedHorarioFormData, selectedGroup.id_grupo);
      }
      ...
      <form onSubmit={handleSubmit}>
        <div className='flex flex-col justify-center'>
          <motion.div variants={itemVariants} className='flex flex-col gap-y-4 mb-4'>
            {inputs.map((input) => (
              <motion.div key={input.id} className="relative" animate={{ scale: focusedField === input.id ? 1.02 : 1 }}>
                <div className={`absolute left-4 top-1/2 transform -translate-y-1/2 transition-colors duration-200 ${focusedField === input.id ? 'text-theme-3 scale-110' : 'text-slate-400'}`}>
                  {React.createElement(input.icon, { className: 'w-5 h-5' })}
                </div>
                <input
                  type={input.type}
                  id={input.id}
                        ...
                        {isDayActive && (
                          ...
                          >
                            <div className="flex-1">
                              <label htmlFor={`inicio-${diaId}`} className="block text-sm font-medium text-gray-700 mb-1">Hora de Inicio:</label>
                              <input
                                type="time"
                                id={`inicio-${diaId}`}
                            ...
                            <div className="flex-1">
                              <label htmlFor={`fin-${diaId}`} className="block text-sm font-medium text-gray-700 mb-1">Hora de Fin:</label>
                              <input
                                type="time"
                                id={`fin-${diaId}`}
